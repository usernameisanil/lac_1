...
| 2 | **Types of Matrices** | Dedicated `infobox`: define ALL types — Square, Rectangular, Row, Column, Null/Zero, Identity, Diagonal, Scalar, Upper Triangular, Lower Triangular. Worked Example: classify a set of 5 given matrices into their types with justification. |
| 3 | **Matrix Operations** | Dedicated `infobox`: define Addition (same order), Scalar Multiplication, Matrix Multiplication (conformability condition m×n · n×p = m×p), with non-commutativity warning AB ≠ BA. Worked Example: compute A+B, A−B, 3A, AB, BA explicitly showing every arithmetic step. |
...
**ENFORCEMENT RULE:** The document must contain exactly 7 named `\subsection{}` entries (one per sub-topic above) inside the main definitions section. The assessment section must contain at least one viva question AND one MCQ explicitly targeting each of the 7 sub-topics above.
...
1. **Algorithm: Classify a Matrix** — decision tree (check order → check diagonal → check symmetry → output type).
...
Classify each of 5 given matrices (2×2, 3×3, 1×4, 3×3 diagonal, 3×3 upper triangular) — state type with full justification for every classification.
`learnbox`: "Every matrix belongs to at least one type — check conditions systematically."

**Example 3 — Matrix Operations:**
Given A = [[1,2],[3,4]], B = [[5,6],[7,8]]: compute A+B, A−B, 3A, AB, BA. Show AB ≠ BA with explicit arithmetic. State conformability check before multiplication.
...
`booktabs` table: Matrix Type | Definition/Condition | Example (2×2 or 3×3) | Engineering Occurrence — covering all 10+ types from Sub-Topic 2 plus symmetric and skew-symmetric.
...
2. [Sub-topic 2] What is the difference between a diagonal matrix and a scalar matrix?
3. [Sub-topic 3] Why is matrix multiplication not commutative? Give counterexample.
4. [Sub-topic 3] What is the conformability condition for AB to exist?
5. [Sub-topic 4] State ALL four properties of transpose.
6. [Sub-topic 5] What is the condition for a matrix to have an inverse?
7. [Sub-topic 5] State the property (AB)^{-1} = ?
8. [Sub-topic 6] Can a rectangular matrix be symmetric? Why?
9. [Sub-topic 7] Why must all diagonal entries of a skew-symmetric matrix be zero?

**Descriptive Exam Questions (5 Problems — covering all sub-topics):**
1. Classify and justify types of 5 given matrices.
2. Compute AB, BA, (AB)^T and verify reversal rule.
...
- Types: memorize 10 types and their conditions
